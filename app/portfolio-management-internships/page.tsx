...
import {
  ...
  Calendar, Briefcase, TrendingUp, Bell, Wallet, PieChart
} from "lucide-react";
...
const topCompanies = [
  "BlackRock", "Fidelity Investments", "Vanguard", "HDFC AMC",
  "SBI Mutual Fund", "ICICI Prudential AMC", "Nippon India MF", "Axis AMC"
];

const locations = ["Mumbai", "Bengaluru", "Delhi NCR", "Chennai", "Pune", "Hyderabad", "Kolkata", "Remote"];
    ...
    try {
      ...
      if (Array.isArray(data)) {
        const filtered = data.filter((job: Internship) => 
          ...
          job.category?.toLowerCase().includes("portfolio management") ||
          job.category?.toLowerCase().includes("asset management") ||
          job.title?.toLowerCase().includes("portfolio") ||
          job.title?.toLowerCase().includes("asset management") ||
          job.title?.toLowerCase().includes("mutual fund") ||
          job.title?.toLowerCase().includes("wealth management")
        );
      ...
      <main className="bg-[#F8FAFC] min-h-screen">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <nav className="flex items-center gap-2 text-sm text-slate-400 mb-6">
            ...
            <span className="text-[#0A2540] font-medium">Portfolio Management Internships</span>
          </nav>

          <div className="text-center mb-8">
            <div className="inline-flex items-center gap-2 bg-[#0A2540]/5 px-4 py-2 rounded-full mb-5">
              <span className="w-1.5 h-1.5 bg-[#10B981] rounded-full animate-pulse" />
              <span className="text-xs font-medium text-[#0A2540]">PMS & AMC</span>
            </div>
            <h1 className="text-4xl sm:text-5xl font-bold text-[#0A2540] mb-4">
              Portfolio Management Internships in India <span className="text-[#10B981]">(2026)</span>
            </h1>
            <p className="text-slate-500 max-w-2xl mx-auto">
              Verified portfolio management internships at top AMCs - no signup required.
            </p>
          </div>

          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 max-w-3xl mx-auto mb-12">
            <div className="text-center">
              <div className="text-2xl font-bold text-[#0A2540]">{internships.length || 7}+</div>
              <div className="text-xs text-slate-400">Active Roles</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-[#0A2540]">15+</div>
              <div className="text-xs text-slate-400">AMCs Hiring</div>
            </div>
            <div className="text-center">
          ...
          <div className="mb-12">
            <div className="text-center mb-6">
              <h2 className="text-xl font-semibold text-[#0A2540]">Latest Portfolio Management Internships</h2>
            ...
            {isLoading ? (
              ...
              <div className="bg-white rounded-xl border border-slate-200 p-10 text-center">
                ...
                <p className="text-slate-500">No portfolio management internships available right now.</p>
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-6 mb-6">
            <div className="flex items-center gap-3 mb-4">
              ...
              <h2 className="text-lg font-semibold text-[#0A2540]">Top AMCs Hiring for Portfolio Management</h2>
            </div>
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-6 mb-6">
            ...
            <div className="space-y-4">
              <div>
                <h3 className="text-sm font-semibold text-[#0A2540] mb-1">What is portfolio management?</h3>
                <p className="text-xs text-slate-500">Portfolio management involves building and managing investment portfolios for clients, including asset allocation, security selection, risk management, and performance monitoring to achieve specific financial goals.</p>
              </div>
              <div>
                <h3 className="text-sm font-semibold text-[#0A2540] mb-1">How much do portfolio management interns earn in India?</h3>
                <p className="text-xs text-slate-500">Portfolio management interns typically earn between Rs.25,000 to Rs.70,000 per month depending on the AMC and role.</p>
              </div>
              <div>
                <h3 className="text-sm font-semibold text-[#0A2540] mb-1">What skills are needed for portfolio management internships?</h3>
                <p className="text-xs text-slate-500">Investment analysis, asset allocation, risk management, financial modeling, Excel, knowledge of mutual funds, equities, bonds, and alternative investments.</p>
              </div>
              <div>
                <h3 className="text-sm font-semibold text-[#0A2540] mb-1">Which AMCs hire portfolio management interns in India?</h3>
                <p className="text-xs text-slate-500">Top AMCs include BlackRock, Fidelity, Vanguard, HDFC AMC, SBI Mutual Fund, ICICI Prudential AMC, Nippon India MF, and Axis AMC.</p>
              </div>
              <div>
                <h3 className="text-sm font-semibold text-[#0A2540] mb-1">What certifications help for portfolio management careers?</h3>
                <p className="text-xs text-slate-500">CFA, FRM, CFP, NISM certifications, and MBA in Finance are highly valued for portfolio management roles.</p>
              </div>
              <div>
                <h3 className="text-sm font-semibold text-[#0A2540] mb-1">Do portfolio management internships lead to full-time jobs?</h3>
                <p className="text-xs text-slate-500">Yes, many AMCs offer pre-placement offers (PPOs) to high-performing portfolio management interns for full-time analyst roles.</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-slate-50 rounded-xl p-5 mb-6">
            ...
            <div className="flex flex-wrap justify-center gap-2">
              ...
              <Link href="/risk-compliance-internships" className="px-3 py-1.5 bg-white border border-slate-200 rounded-full text-xs text-slate-500 hover:border-[#10B981] hover:text-[#10B981] transition">Risk & Compliance</Link>
            </div>
          </div>

          <div className="bg-[#0A2540] rounded-xl p-8 text-center">
            <h3 className="text-white text-xl font-semibold mb-2">Ready to start your portfolio management career?</h3>
