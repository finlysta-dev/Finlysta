...
const topCompanies = [
  "Morgan Stanley", "Goldman Sachs", "J.P. Morgan", "Kotak Securities",
  "ICICI Securities", "HDFC Securities", "Motilal Oswal", "Axis Securities"
];

const locations = ["Mumbai", "Bengaluru", "Delhi NCR", "Kolkata", "Remote"];
...
export default function EquityResearchPage() {
  ...
  useEffect(() => {
    fetchEquityResearchInternships();
  }, []);

  const fetchEquityResearchInternships = async () => {
    setIsLoading(true);
    try {
      ...
      if (Array.isArray(data)) {
        const filtered = data.filter((job: Internship) => 
          job.category === "Equity Research" || 
          job.category?.toLowerCase().includes("equity research") ||
          job.title?.toLowerCase().includes("equity research")
        );
      ...
      <main className="bg-[#F8FAFC] min-h-screen">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <nav className="flex items-center gap-2 text-sm text-slate-400 mb-6">
            ...
            <span className="text-[#0A2540] font-medium">Equity Research Internships</span>
          </nav>
          ...
          <div className="text-center mb-8">
            <div className="inline-flex items-center gap-2 bg-[#0A2540]/5 px-4 py-2 rounded-full mb-5">
              ...
              <span className="text-xs font-medium text-[#0A2540]">High Demand</span>
            </div>
            <h1 className="text-4xl sm:text-5xl font-bold text-[#0A2540] mb-4">
              Equity Research Internships in India <span className="text-[#10B981]">(2026)</span>
            </h1>
            <p className="text-slate-500 max-w-2xl mx-auto">
              Verified equity research internships at top firms — no signup required.
            </p>
          </div>
          ...
          <div className="mb-12">
            <div className="text-center mb-6">
              <h2 className="text-xl font-semibold text-[#0A2540]">Latest Equity Research Internships</h2>
            ...
            {isLoading ? (
              ...
              <div className="bg-white rounded-xl border border-slate-200 p-10 text-center">
                ...
                <p className="text-slate-500">No equity research internships available right now.</p>
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-6 mb-6">
            <div className="flex items-center gap-3 mb-4">
              ...
              <h2 className="text-lg font-semibold text-[#0A2540]">Top Companies Hiring Equity Research Interns</h2>
            </div>
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-6 mb-6">
            ...
            <div className="space-y-4">
              <div>
                <h3 className="text-sm font-semibold text-[#0A2540] mb-1">What is equity research?</h3>
                <p className="text-xs text-slate-500">Equity research involves analyzing companies, industries, and market trends to provide investment recommendations to institutional investors and fund managers.</p>
              </div>
              <div>
                <h3 className="text-sm font-semibold text-[#0A2540] mb-1">How much do equity research interns earn in India?</h3>
                <p className="text-xs text-slate-500">Equity research interns typically earn between ₹20,000 to ₹50,000 per month depending on the firm.</p>
              </div>
              <div>
                <h3 className="text-sm font-semibold text-[#0A2540] mb-1">What skills are needed for equity research?</h3>
                <p className="text-xs text-slate-500">Financial modeling, valuation, Excel, accounting, sector knowledge, and strong writing skills for research reports.</p>
              </div>
              <div>
                <h3 className="text-sm font-semibold text-[#0A2540] mb-1">Do equity research internships lead to full-time jobs?</h3>
                <p className="text-xs text-slate-500">Yes, many firms offer pre-placement offers (PPOs) to high-performing interns for full-time research analyst roles.</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-[#0A2540] rounded-xl p-8 text-center">
            <h3 className="text-white text-xl font-semibold mb-2">Ready to start your equity research career?</h3>
