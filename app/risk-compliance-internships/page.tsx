...
import {
  ...
  Calendar, Briefcase, TrendingUp, Bell, Wallet, Shield
} from "lucide-react";
...
const topCompanies = [
  "J.P. Morgan", "Goldman Sachs", "Morgan Stanley", "Deutsche Bank",
  "HSBC", "HDFC Bank", "ICICI Bank", "KPMG"
];
    ...
    try {
      ...
      if (Array.isArray(data)) {
        const filtered = data.filter((job: Internship) => 
          ...
          job.category?.toLowerCase().includes("risk") ||
          job.category?.toLowerCase().includes("compliance") ||
          job.title?.toLowerCase().includes("risk") ||
          job.title?.toLowerCase().includes("compliance") ||
          job.title?.toLowerCase().includes("credit risk") ||
          job.title?.toLowerCase().includes("market risk")
        );
      ...
      <main className="bg-[#F8FAFC] min-h-screen">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <nav className="flex items-center gap-2 text-sm text-slate-400 mb-6">
            ...
            <span className="text-[#0A2540] font-medium">Risk & Compliance Internships</span>
          </nav>

          <div className="text-center mb-8">
            <div className="inline-flex items-center gap-2 bg-[#0A2540]/5 px-4 py-2 rounded-full mb-5">
              <span className="w-1.5 h-1.5 bg-[#10B981] rounded-full animate-pulse" />
              <span className="text-xs font-medium text-[#0A2540]">Banking Sector</span>
            </div>
            <h1 className="text-4xl sm:text-5xl font-bold text-[#0A2540] mb-4">
              Risk & Compliance Internships in India <span className="text-[#10B981]">(2026)</span>
            </h1>
            <p className="text-slate-500 max-w-2xl mx-auto">
              Verified risk management and compliance internships at top banks - no signup required.
            </p>
          </div>

          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 max-w-3xl mx-auto mb-12">
            <div className="text-center">
              <div className="text-2xl font-bold text-[#0A2540]">{internships.length || 8}+</div>
              <div className="text-xs text-slate-400">Active Roles</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-[#0A2540]">20+</div>
              <div className="text-xs text-slate-400">Banks Hiring</div>
            </div>
            <div className="text-center">
          ...
          <div className="mb-12">
            <div className="text-center mb-6">
              <h2 className="text-xl font-semibold text-[#0A2540]">Latest Risk & Compliance Internships</h2>
            ...
            {isLoading ? (
              ...
              <div className="bg-white rounded-xl border border-slate-200 p-10 text-center">
                ...
                <p className="text-slate-500">No risk and compliance internships available right now.</p>
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-6 mb-6">
            <div className="flex items-center gap-3 mb-4">
              ...
              <h2 className="text-lg font-semibold text-[#0A2540]">Top Banks Hiring for Risk & Compliance</h2>
            </div>
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-6 mb-6">
            ...
            <div className="space-y-4">
              <div>
                <h3 className="text-sm font-semibold text-[#0A2540] mb-1">What is risk management in banking?</h3>
                <p className="text-xs text-slate-500">Risk management involves identifying, assessing, and mitigating financial risks including credit risk, market risk, operational risk, and regulatory compliance in banking and financial institutions.</p>
              </div>
              <div>
                <h3 className="text-sm font-semibold text-[#0A2540] mb-1">How much do risk interns earn in India?</h3>
                <p className="text-xs text-slate-500">Risk management interns typically earn between Rs.25,000 to Rs.60,000 per month depending on the bank and role.</p>
              </div>
              <div>
                <h3 className="text-sm font-semibold text-[#0A2540] mb-1">What skills are needed for risk management internships?</h3>
                <p className="text-xs text-slate-500">Financial risk analysis, statistical modeling, Excel, SQL, understanding of Basel norms, regulatory frameworks, and strong analytical thinking.</p>
              </div>
              <div>
                <h3 className="text-sm font-semibold text-[#0A2540] mb-1">Which banks hire risk management interns in India?</h3>
                <p className="text-xs text-slate-500">Top banks include J.P. Morgan, Goldman Sachs, Morgan Stanley, Deutsche Bank, HSBC, HDFC Bank, ICICI Bank, and consulting firms like KPMG.</p>
              </div>
              <div>
                <h3 className="text-sm font-semibold text-[#0A2540] mb-1">What is the difference between credit risk and market risk?</h3>
                <p className="text-xs text-slate-500">Credit risk is the risk of borrower default, while market risk is the risk of losses from market movements like interest rates, exchange rates, and stock prices.</p>
              </div>
              <div>
                <h3 className="text-sm font-semibold text-[#0A2540] mb-1">Do risk internships lead to full-time jobs?</h3>
                <p className="text-xs text-slate-500">Yes, many banks offer pre-placement offers (PPOs) to high-performing risk management interns for full-time analyst roles.</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-[#0A2540] rounded-xl p-8 text-center">
            <h3 className="text-white text-xl font-semibold mb-2">Ready to start your risk management career?</h3>
